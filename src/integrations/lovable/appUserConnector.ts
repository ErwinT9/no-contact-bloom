 ...
 *
 * Server-only. Only import this from `createServerFn` handlers (dynamically,
 * so it never enters a browser bundle) — it reads server-side secrets and
 * talks to the Lovable connector gateway directly.
 */
...
/** A gateway 401 whose body type starts with "credential_" means re-authorization. */
